...
public class PropertyHelperTests
{
    ...
    public void CreatePropertySelector_WithExistingPropertyName_ShouldReturnPropertySelectorExpression()
    {
        ...
        result.Parameters[0].Should().BeAssignableTo<ParameterExpression>();
        result.Body.Should().BeAssignableTo<UnaryExpression>();
        result.Body.NodeType.Should().Be(ExpressionType.Convert);
        result.Body.Type.Should().Be<object>();
        var unaryExpression = (UnaryExpression)result.Body;
        unaryExpression.Operand.Should().BeAssignableTo<MemberExpression>();
