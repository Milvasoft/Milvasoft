...
public class ContextRepository<TContext>(TContext dbContext) : IContextRepository where TContext : DbContext, IMilvaDbContextBase
{
    ...
    /// <returns></returns>
    public Task ApplyTransactionAsync(Func<Task> function, bool startTransaction = true)
    {
        if (startTransaction)
        {
    ...
    /// <param name="startTransaction"> When nested conditional transactions are desired, a transaction cannot be started for the transaction it contains. </param>
    /// <returns></returns>
    public Task ApplyTransactionAsync(Func<Task> function, Func<Task> rollbackFunction, bool startTransaction = true)
    {
        if (startTransaction)
        {
            var executionStrategy = _dbContext.Database.CreateExecutionStrategy();
    ...
    /// <param name="startTransaction"> When nested conditional transactions are desired, a transaction cannot be started for the transaction it contains. </param>
    /// <returns></returns>
    public Task ApplyTransactionAsync(Func<Task> function, Action rollbackFunction, bool startTransaction = true)
    {
        if (startTransaction)
        {
            var executionStrategy = _dbContext.Database.CreateExecutionStrategy();
